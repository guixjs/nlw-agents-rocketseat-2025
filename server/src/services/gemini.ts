...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  ...
  if (!response.text) {
    throw new Error('Não foi possível trascrever o áduio!')
  }
...
export async function generateEmbeddings(text: string) {
  ...
  if (!response.embeddings?.[0].values) {
    throw new Error('Não foi posível gerar os embeddings')
  }
...
export async function generateAnswer(question: string, trascriptions: string[]) {
  const contexto = trascriptions.join("\n\n")
  const prompt = `
